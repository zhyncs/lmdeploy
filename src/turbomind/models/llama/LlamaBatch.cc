...
void LlamaBatch<T>::MedusaVerify(const int inited_count, const int max_init_ctx_len, const int batch_size)
{
    ...
    if (inited_count != 0) {
        ...
        check_cuda_error(cudaStreamSynchronize(stream_));
        medusa_utils_->path_tree_.getBatchedOutputIds(inited_input_ids.data(),
                                                      unpacked_input_ids.data(),
                                                      unpacked_each_path_len.data(),
                                                      medusa_num_heads_,
                                                      inited_count);
        medusa_utils_->path_tree_.getBatchedOutputIds(
            ref_output_ids.data(), unpacked_output_ids.data(), nullptr, medusa_num_heads_, inited_count);
        ...
        medusa_utils_->path_tree_.getBatchedLastMatchIdx(
            h_medusa_max_match_idx_buf_, h_medusa_max_match_length_buf_, h_medusa_last_match_idx_buf_, inited_count);

        medusa_utils_->path_tree_.getBatchedMatchedPartIdx(h_medusa_max_match_idx_buf_,
                                                           h_medusa_max_match_length_buf_,
                                   ...
                                   bool&     medusa_should_stop)
{
    ...
    medusa_utils_->path_tree_.getBatchedPseudoIdsFromTree(h_medusa_preds_batched_buf_,
                                                          medusa_num_heads_,
                                                          medusa_top_k_,
                                                          h_medusa_max_match_length_buf_,
                                                          h_medusa_max_match_idx_buf_,
