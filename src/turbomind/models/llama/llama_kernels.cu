...
void invokeMedusaBatchedMatchKernel(const int*   output_ids1,
                                    const int*   output_ids2,
                                    int*         max_match_idx,
...
{
    // inputs:
    // output_ids1: [batch_size, path_num, 1 + medusa_head_num]
    // output_ids2: [batch_size, path_num, 1 + medusa_head_num]
    // outputs:
    ...
    medusaBatchedMatchKernel<64><<<grid, block, 0, stream>>>(
        output_ids1, output_ids2, max_match_idx, max_match_length, path_num, medusa_head_num);
}
